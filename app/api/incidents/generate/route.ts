...
export async function POST() {
  ...
  try {
    ...
    console.log("Gemini incident generation failed, falling back to Kimi:", (geminiError as Error).message)
    try {
      ...
      if (!parsed) {
        console.log("Kimi incident generation returned unparseable JSON")
        return NextResponse.json({ ok: false, provider: "Offline", generated: 0 }, { status: 200 })
      }
      result = { object: parsed }
    } catch (kimiError) {
      console.log("Kimi incident generation also failed:", (kimiError as Error).message)
      return NextResponse.json({ ok: false, provider: "Offline", generated: 0 }, { status: 200 })
    }
  }
  ...
  if (rows.length > 0) {
    ...
    if (insertError) {
      console.log("incidents insert error:", insertError.message)
