...
async function fetchWeather(lat: number, lng: number): Promise<LiveWeather> {
  try {
    ...
    console.log("city-intel weather fetch failed:", (err as Error).message)
...
export async function GET(request: Request) {
  ...
  try {
    return await withModel(
      async () => (await generateText({ model: GEMINI_MODEL, prompt, temperature: 0.4 })).text,
      "Gemini",
      GEMINI_MODEL,
    )
  } catch (geminiError) {
    console.log("city-intel Gemini failed, trying Kimi:", (geminiError as Error).message)
    try {
      return await withModel(
      ...
      console.log("city-intel Kimi failed:", (kimiError as Error).message)
