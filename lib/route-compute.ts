...
export async function computeDrivingRoute(origin: LatLng, destination: LatLng) {
  ...
  for (const apiKey of keys) {
    try {
      ...
      if (!res.ok) {
        console.log("[v0] computeDrivingRoute error:", res.status, (await res.text()).slice(0, 200))
        continue
      }
      ...
      console.log("[v0] computeDrivingRoute exception:", (err as Error).message)
    }
  }
  return null
}
